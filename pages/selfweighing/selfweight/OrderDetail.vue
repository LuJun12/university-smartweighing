    ...
    <div class="yq-flex-column staff-detail" v-loading="detailInfo.loading">
            ...
            <el-card class="box-card">
                <div class="thyj">
                    <div class="type-left">
                        ...
                        <el-tag v-if="detailInfo.info.refundStatusName">{{ detailInfo.info.refundStatusName }}</el-tag>
                    </div>
        ...
        <div class="tab-all">
            ...
            <el-tabs v-model="activeName" v-if="detailInfo.info.orderStatus !== 4">
                ...
                <el-tab-pane label="打菜日志" name="second" v-if="detailInfo.info.orderStatus === 1">
                    <el-table :data="dataListLog" style="width: 100%; margin-top: 20px">
                        <el-table-column v-for="(item, index) in columnslog" :prop="item.prop" :key="index" :formatter="item.formatter" :label="item.label" :width="item.width">
                            <template #default="scope">
                                <template v-if="item.formatter">
                                    <span v-html="item.formatter(scope.row)"></span>
                                </template>
...
import RefundDilog from './components/Refund.vue'
...
const columnslog = ref([
    { prop: 'actionTime', label: '打餐时间', width: '' },
    { prop: 'itemName', label: '菜品', width: '' },
    { prop: 'itemWeightChange', label: '重量变化（g）', width: '' },
    { prop: 'orderTotalWeight', label: '订单总重（g）', width: '' },
    { prop: 'orderDishPrice', label: '价格变化（元）', width: '' },
    { prop: 'orderTotalPrice', label: '订单总价（元）', width: '' },
    { prop: 'deviceName', label: '相关设备', width: '' },
])
const columnstk = ref([
